export const obtenerUsuarioPorId = async (idUsuario) => {
    try {
        const response = await fetch(`http://192.168.1.202:5000/usuario/buscar/${idUsuario}`)
...
export const crearUnionConOperario = async (dataInfo) => {
    try {
        ...
        const response = await fetch(`http://192.168.1.202:5000/usuario/crearUnionConOperario`, {
            method: 'POST',
            headers: {
...
export const crearAdmin = async (dataInfo) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://192.168.1.202:5000/usuario/crearAdmin`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
...
export const eliminarAdmin = async (idAdmin) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://192.168.1.202:5000/usuario/eliminarAdmin/${idAdmin}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
