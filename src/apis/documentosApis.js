export const obtenerTodosLosDocumentosPorIdContrato = async (idContrato) => {
    try {
        const response = await fetch(`http://127.0.0.1:5000/documento/buscarPorContrato/${idContrato}`);
...
export const obtenerElArchivoPorUrl = async (url) => {
    try {
        const response = await fetch(`http://127.0.0.1:5000/documento/archivos/${url}`)
...
export const editarEstadoDelDocumento = async (idDocumento, estado) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://127.0.0.1:5000/documento/cambiar_estado/${idDocumento}`, {
            method: "PUT",
            headers: {
                Authorization: `Bearer ${token}`,
...
export const generarPdfDelOperario = async (contrato_id) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://127.0.0.1:5000/documento/pdf/${contrato_id}`, {
            method: "GET",
            headers: {
                Authorization: `Bearer ${token}`,
            },
