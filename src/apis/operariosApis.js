export const obtenerOperarioPorId = async (idOperario) => {
    try {
        const response = await fetch(`http://127.0.0.1:5000/operario/buscar/${idOperario}`)
...
export const obtenerTodosLosOperarios = async () => {
    try {
        ...
        const response = await fetch(`http://127.0.0.1:5000/operario/obtenerTodos`, {
            method: "GET",
            headers: {
...
export const crearOperarioConDatosBasicos = async (formData) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://127.0.0.1:5000/operario/crearOperarioRelacionandoTodo`, {
            method: "POST",
            headers: {
...
export const actualizarOperarioPorId = async (formData) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://127.0.0.1:5000/operario/actualizar/${formData.idOperario}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
...
export const filtrarOperariosCompleto = async (filtros) => {
    try {
        ...
        const response = await fetch(`http://127.0.0.1:5000/operario/filtrarOperariosCompleto`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
