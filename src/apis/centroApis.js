export const handlerObtenerCentroPorEmpresaId = async (idEmpresa) => {
    try {
        const response = await fetch(`http://127.0.0.1:5000/centroTrabajo/obtener/${idEmpresa}`);
...
export const crearNuevoCentroConEmpresa = async (formData) => {
    try {
        ...
        const response = await fetch("http://127.0.0.1:5000/centroTrabajo/crear", {
            method: "POST",
            headers: {
...
export const eliminarCentroPorId = async (idCentro) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://127.0.0.1:5000/centroTrabajo/eliminar/${idCentro}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
...
export const actualizarCentroPorId = async (formData) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://127.0.0.1:5000/centroTrabajo/actualizar/${formData.id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
