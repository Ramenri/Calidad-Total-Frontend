export const obtenerEmpresaPorId = async (idEmpresa) => {
    try {
        const response = await fetch(`http://192.168.1.202:5000/empresa/buscarPorUuid?uuid=${idEmpresa}`);
...
export const obtenerTodasLasEmpresas = async () => {
    try {
        const response = await fetch(`http://192.168.1.202:5000/empresa/todo`);
        const data = await response.json();
            return data;
    } catch (error) {
...
export const crearNuevaEmpresa = async (empresa) => {
    try {
        ...
        const response = await fetch(`http://192.168.1.202:5000/empresa/crear`, {
            method: 'POST',
            headers: {
...
export const actualizarEmpresaPorId = async (empresa) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://192.168.1.202:5000/empresa/actualizar/${empresa.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
...
export const obtenerEmpresasDelOperarioNoAfiliadas = async (cedulaOperario) => {
    try {
        const response = await fetch(`http://192.168.1.202:5000/empresa/obtenerEmpresasDelOperarioNoAfiliadas/${cedulaOperario}`);
        const data = await response.json();
        return data;
    } catch (error) {
