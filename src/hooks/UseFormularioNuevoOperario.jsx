...
export const useFormularioNuevoOperario = (buscarOperariosCallback) => {
  ...
  const guardarNuevoOperario = async (e) => {
    ...
    try {
      const operarioResponse = await fetchConManejoErrores("http://192.168.1.202:5000/operario/crear", {
        method: "POST",
        headers: {
      ...
      const contratoResponse = await fetchConManejoErrores("http://192.168.1.202:5000/contrato/crear", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (operarioId) {
        try {
          await fetchConManejoErrores(`http://192.168.1.202:5000/operario/eliminar/${operarioId}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
