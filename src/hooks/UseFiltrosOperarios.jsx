...
export const UseFiltrosOperarios = () => {
    ...
    const fetchEmpresas = async () => {
        try {
            const data = await fetchConManejoErrores("http://127.0.0.1:5000/empresa/todo");
            setEmpresas(data);
        } catch (error) {
    ...
    const fetchCargos = async () => {
        try {
            const data = await fetchConManejoErrores("http://127.0.0.1:5000/contrato/cargos");
            setCargos(data);
        } catch (error) {
    ...
    const buscarOperarios = async () => {
        try {
            ...
            const data = await fetchConManejoErrores("http://127.0.0.1:5000/operario/filtrarOperarios", {
                method: "POST",
    ...
    const buscarOperariosConEmpresa = async (empresaId) => {
        try {
            ...
            const data = await fetchConManejoErrores(`http://127.0.0.1:5000/operario/operariosPorEmpresa/${empresaId}`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
